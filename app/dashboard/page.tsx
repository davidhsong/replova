...
function StatusBadge({ status }: { status: string | null }) {
  ...
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
      emailed ? 'bg-emerald-950 text-emerald-400 border border-emerald-900' : 'bg-zinc-800 text-zinc-400 border border-zinc-700'
    }`}>
        ...
        <div className="flex items-center justify-center px-4 py-24">
          <div className="max-w-sm w-full text-center">
            ...
            <p className="text-zinc-400 text-sm leading-relaxed mb-8">
              Activate your free trial to start receiving AI reply drafts for {restaurant.name} every Monday.
            </p>
            <a
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        {params.success && (
          <div className="mb-6 flex items-center gap-3 bg-emerald-950/60 border border-emerald-900 rounded-lg px-4 py-3">
            ...
            <p className="text-sm text-emerald-300">You're all set. Your first digest arrives next Monday morning.</p>
          </div>
        )}
        ...
        {!reviews || reviews.length === 0 ? (
          <div className="border border-zinc-800 rounded-xl p-12 text-center">
            ...
            <p className="text-zinc-500 text-sm">Your first weekly digest will arrive next Monday morning.</p>
          </div>
        ) : (
          <div className="space-y-5">
            {reviews.map((review) => (
              <div key={review.id} className="border border-zinc-800 rounded-xl p-5 bg-zinc-900/40">
